...
-- RegisterCommand("ton_skate", function()
-- 	RCCar.Start()
-- end)


AddEventHandler('ton_skate:clear', function()
	RCCar.Clear()
end)

AddEventHandler('ton_skate:useItem', function()
	TriggerServerEvent('ton_skate:useItem')
    RCCar.Start()
end)
...
RCCar.HandleKeys = function(distanceCheck)
	if distanceCheck <= 1.5 then
		if IsControlJustPressed(0, 38) then
			TriggerServerEvent('ton_skate:unUseItem')
			RCCar.Attach("pick")
		end
...
RegisterNetEvent("ton_skate:shareHeIsOnSkate")
AddEventHandler("ton_skate:shareHeIsOnSkate", function(id) 
